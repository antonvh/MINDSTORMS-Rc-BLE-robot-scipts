...
class BLESimplePeripheral:
    ...
    def text_to_speech(self,s):
        if self._connected:
            self.send("S"+s)

    def set_left_slider(self,val):
        if self._connected:
            if val>360: val=360
            elif val<-360: val=-360
            self.send("L%d"%val)

    def set_right_slider(self,val):
        if self._connected:
            if val>360: val=360
            elif val<-360: val=-360
            self.send("R%d"%val)
...
def on_rx(control):
    # update global variables containing controller values
...
t1=time.ticks_ms() # timer fot animation on lcd screen
t2=time.ticks_ms() # timer for showing options in status line
# Control loop
n=0

status_lines=["Press 1 to start animation in LCD",
            "Press 2 to stgop animation in LCD",
            "Press 3 to set right slider",
            "Press 4 to set left slider",
            "Press 5 to test text-to-speech"]
status_n=0
...
while True:
    ...
    if receiver.is_connected():
        if time.ticks_diff(time.ticks_ms(),t2)>2000:
            t2=time.ticks_ms()
            print(status_lines[status_n%15])
            receiver.show_status(status_lines[status_n%15])
            status_n=(status_n+1)%5
        ...
        if show_animation:
            if time.ticks_diff(time.ticks_ms(),t1)>100:
                ...
                try:
                    ...
                    receiver.send(repr(_CONNECT_IMAGES[anim_n]))
                    anim_n=(anim_n+1)%15
                except:
                    pass
        ...
        if buttons[1]:
            buttons[1]=0
            show_animation=False
            receiver.send(repr(receiver._logo))

        if buttons[2]:
            buttons[2]=0
            receiver.set_right_slider(l_slider)

        if buttons[3]:
            buttons[3]=0
            receiver.set_left_slider(r_slider)

        if buttons[4]:
            buttons[4]=0
            receiver.text_to_speech("This is text to speech.")
